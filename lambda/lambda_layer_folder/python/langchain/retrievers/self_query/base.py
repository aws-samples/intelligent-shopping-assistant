...
def _get_builtin_translator(vectorstore: VectorStore) -> Visitor:
    ...
    elif str(vectorstore.__class__).replace("<class","").replace("'>","").split('.')[1] == "OpenSearchVectorSearch":
        return OpenSearchTranslator()
    elif str(vectorstore.__class__).replace("<class","").replace("'>","").split('.')[1] == "MyScale":
