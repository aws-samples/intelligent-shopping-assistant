...
def data_load(invoke_url,index,file_name,load_type,text_coloum_name: str='',llm_keywords: str='',image_coloum_name: str='',text_embedding_model: str='',image_embedding_model: str='',text_endpoint_name: str='',image_endpoint_name: str='',model_id: str='',api_url: str=''):
    url = invoke_url + '/data_load?'
    url += ('&index='+index)
    url += ('&task=data_load')
    ...
    if load_type.find('text') >= 0:
        if len(text_embedding_model) > 0:
            url += ('&textEmbeddingModelId='+text_embedding_model)
        elif len(text_endpoint_name) > 0:
                url += ('&textEmbeddingEndpoint='+text_endpoint_name)
    
    if load_type.find('image') >= 0:
        if len(image_embedding_model) > 0:
            url += ('&imageEmbeddingModelId='+image_embedding_model)
        elif len(image_endpoint_name) > 0:
...
with st.sidebar:
    ...
    if load_type.find('text') >=0:
        model_type  = st.radio("Text Model Type",["Bedrock","Sagemaker"])
        if model_type == 'Bedrock':
            model_id_list = ['amazon.titan-embed-text-v2:0','amazon.titan-embed-image-v1','cohere.embed-multilingual-v3','cohere.embed-english-v3']
            text_embedding_model = st.selectbox("Please Select text embedding model", model_id_list)
        ...
        llm_keywords  = st.radio("Use LLM to extract keywords",["No","Yes"])
        model_id = ''
        if llm_keywords == "Yes":
            LLM_model_list = ["anthropic.claude-3-haiku-20240307-v1:0","anthropic.claude-3-sonnet-20240229-v1:0"]
            model_id = st.selectbox("Select the LLM model",LLM_model_list)

    image_coloum_name = ''
    image_endpoint_name = ''
    image_embedding_model = ''
    if load_type.find('image') >=0:
        image_model_type  = st.radio("Image Model Type",["Bedrock","Sagemaker"])
        if image_model_type == 'Bedrock':
            image_model_id_list = ['amazon.titan-embed-image-v1']
            image_embedding_model = st.selectbox("Please Select text embedding model", image_model_id_list)

        elif image_model_type == 'Sagemaker':
            if len(sagemaker_endpoint) == 0:
                sagemaker_endpoint = get_sagemaker_endpoint(invoke_url)
    ...
    size = 10
    if load_type.find('text') >=0 and llm_keywords == 'No':
        size = 80
    elif load_type.find('text') >= 0 and llm_keywords == 'Yes':
        size = 6
            ...
            with st.empty():
                for i in range(len(file_list)):
                    ...
                    if len(split_data) > 0:
                        ...
                        try:
                            response = data_load(invoke_url,index,object_name,load_type,text_coloum_name,llm_keywords,image_coloum_name,text_embedding_model,image_embedding_model,text_endpoint_name,image_endpoint_name,model_id)
